...
RSpec.describe "dashboard next credit card payment", type: :request do
  ...
  it "shows the nearest upcoming credit card payment due date and total" do
    Transaction.create!(
    ...
    expect(response.body).to include("次回クレカ支払い")
    expect(response.body).to include("9/26")
    expect(response.body).to include("¥3,000")
  end

  it "does not show the tile when there are no unpaid credit card transactions" do
    get dashboard_path

    expect(response.body).not_to include("次回クレカ支払い")
  end
end
